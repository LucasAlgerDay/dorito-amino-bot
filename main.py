...
def botidok(data):
    return data.authorId in ('4b95e172-4b8b-4abb-b601-a59311dc9f81', '6d1d2b3e-6a85-4ab9-a268-1eb3dfa1ff3d')

def is_staff(data):
    return data.authorId in ("4b95e172-4b8b-4abb-b601-a59311dc9f81") or data.subClient.is_in_staff(data.authorId)
...
def lolipolice(data):
    ...
    data.subClient.send_message(chatId=data.chatId, embedContent="Cara de Loli .. >.<", embedTitle=f"{data.author}",file=file, fileType="gif")
...
def pixelate(data):
    ...
    data.subClient.send_message(chatId=data.chatId, embedContent="Pixelate .. >.<", embedTitle=f"{data.author}",file=file, fileType="gif")

@client.command("ahegao")
def ahegao(data):
    url = f"https://api.xteam.xyz/randomimage/ahegao?APIKEY=87a1e51c20b50871"
    file = url_like(url)
    data.subClient.send_message(chatId=data.chatId, embedContent="Toma ahegao para ti ñwñ", embedTitle=f"{data.author}",file=file, fileType="image")

@client.command("bts")
def bts(data):
    url = f"https://api.xteam.xyz/randomimage/bts?APIKEY=87a1e51c20b50871"
    file = url_like(url)
    data.subClient.send_message(chatId=data.chatId, embedContent="Toma tu imagen sobre bts...", embedTitle=f"{data.author}",file=file, fileType="image")

@client.command("wallpaperpc")
def wallpaperpc(data):
    url = f"https://api.xteam.xyz/randomimage/wallpaper?APIKEY=87a1e51c20b50871"
    file = url_like(url)
    data.subClient.send_message(chatId=data.chatId, embedContent="Toma tu wallpaper para pc...", embedTitle=f"{data.author}",file=file, fileType="image")
...
def wallpapermovil(data):
    ...
    file = url_like(url)
    data.subClient.send_message(chatId=data.chatId, embedContent="Toma tu wallpaper para movil...", embedTitle=f"{data.author}",file=file, fileType="image")

@client.command("neko")
def neko(data):
    url = f"https://api.xteam.xyz/randomimage/sfwneko?APIKEY=87a1e51c20b50871"
    file = url_like(url)
    data.subClient.send_message(chatId=data.chatId, embedContent="Toma tu neko kawaii UwU...", embedTitle=f"{data.author}",file=file, fileType="image")

@client.command("jahy")
def jahy(data):
    url = f"https://api.xteam.xyz/randomimage/jahy?APIKEY=87a1e51c20b50871"
    file = url_like(url)
    data.subClient.send_message(chatId=data.chatId, embedContent="Toma tu imagen de Jahy Sama...", embedTitle=f"{data.author}",file=file, fileType="image")
...
def perro(data):
    ...
    data.subClient.send_message(chatId=data.chatId, embedContent=f"Toma tu Perrito {data.message}.. >//<", embedTitle=f"{data.author}",file=file, fileType="gif")
...
def gato(data):
    ...
    data.subClient.send_message(chatId=data.chatId, embedContent=f"Toma tu Gatito {data.message}.. >//<", embedTitle=f"{data.author}",file=file, fileType="gif")
...
def zorro(data):
    ...
    data.subClient.send_message(chatId=data.chatId, embedContent=f"Toma tu Zorrito {data.message}.. >//<", embedTitle=f"{data.author}",file=file, fileType="gif")
...
def guiñopatembed(data):
    ...
    data.subClient.send_message(chatId=data.chatId, embedContent=f"le ha dado un guiño a {data.message}.. >//<", embedTitle=f"{data.author}",file=file, fileType="gif")
...
def hello(data):
    dreven = random.choice(["High School DxD","High School DxD Born", "Kaguya Sama Love is War.. >w<", "Komi San.. >.<", "No se lo digas a nadie pero es Euphoria","Boku No Pico"])
    data.subClient.send_message(data.chatId, dreven, replyTo = data.messageId)
...
[c]casarse • divorcio • shipp • abrazar • besar • mejilla • sonrojarse • morder • cosquillas • pat • guiño • timido • alimentar • enamorado • sonreir • llora •""")

@client.command()
def niveles(data):
    data.subClient.send_message(data.chatId, message="""[bc]⤥〃Niveles༴ 

[c]/lvl ==> Muestra tu nivel.""")

@client.command()
def mascota(data):
    data.subClient.send_message(data.chatId, message="""[bc]⤥〃Mascota༴ 
...
[c] •!wiki • habla • img • google • duckgo • tra • askme • reto • stickerimg • stickergif • chiste • fondo • fonttxt • neontxt • version • simp • loli • pixel • horny • menciona • id • disclaimer • comment • global • globallink • sigueme • twitter • sigueme • unfollow • yahoo • tra • tra-en • chat • voz • idlink • version • letra • perro • gato • zorro  • zorro • jahy • wallpaperpc • wallpapermovil • bts""")
...
def casarse(data):
    ...
    data.subClient.send_message(data.chatId, embedContent=f"Yo los declaro, marido y mujer {data.author} y {data.message} puede besar al novio/a.", embedTitle=f"Casamiento de {data.author} y {data.message}", file=file, fileType="gif")
